...
.rst(reset[1]),    // reset 1 for pitch level meter
...
input  wire clk,    
    ...
    output reg [6:0] seg,             // system clock

    input  wire rst,                     // active-high reset

    input  wire signed [15:0] in_y,     // accelerometer X

    input  wire signed [15:0] in_x   // accelerometer Z

);

    //--------------------------------------------------

    // Wires for CORDIC output
...
// From your SPI mo
    ...
    // CORDIC IP instance (module-level)
    ...
    cordic_0 u_cordic (
        ...
        //.aresetn(~rst),                   // Uncomment if IP expects active-low reset

        .s_axis_cartesian_tvalid(1'b1),     // Always provide data

        .s_axis_cartesian_tdata({in_y,in_x                           }), // {Y, X} = {x_in, z_in}
    ...
    // Sequential logic to detect pitch levels
...
always @(posedge clk) begin
    ...
    if (rst) begin
        ...
        seg <= 7'b1111111; // reset

    end else begin

        // First segment

        if ( angle_out >-16'sd8191 && angle_out <= 16'sd8191)

            seg <= 7'b0011101;
 
        // Second segment

        else if (angle_out > -16'sd24576 && angle_out <= -16'sd8191 )

            seg <= 7'b0111100;
 
        // Third segment

        else if ((angle_out > 16'sd24576  && angle_out <= 16'sd32767)||(angle_out > -16'sd32768 &&angle_out <= -16'sd24576 ))

            seg <= 7'b1011100;
 
        // Fourth segment

        else if (angle_out > 16'sd8191  && angle_out <= 16'sd24576)
...
module pitch_detector #(
    ...
    parameter signed THRESHOLD = 16'd500  , 

     parameter signed NEGTHRESHOLD = -16'd500     // minimum angle to trigger level_1
    ...
    input  wire signed [15:0] in_x,     // accelerometer X

    input  wire signed [15:0] in_z,     // accelerometer Z
    ...
    output reg  level_neg1,

    output reg  level_neg2,

    output reg  level_neg3,

    output reg  level_neg4
...
// From your SPI module

reg [15:0] mag_Az;
 
always @(*) begin

    if (in_z[15] == 1'b1) begin

        // If MSB is 1, it's negative. Invert and add 1.

        mag_Az = ~in_z + 1'b1;

    end else begin

        // If positive, magnitude is the same as the raw value.
    ...
    cordic_0 u_cordic (
        ...
        .s_axis_cartesian_tdata({in_x, mag_Az}), // {Y, X} = {x_in, z_in}
    ...
    always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            // Compare angle to thresholds

            //--------------------------------------------------

            // You can adjust the ranges depending on your CORDIC angle units

            if (angle_out >= THRESHOLD && angle_out < 4095)

                level_1 <= 1'b1;

            else if (angle_out >= 4095 && angle_out < 8191)

                level_2 <= 1'b1;

            else if (angle_out >= 8191 && angle_out < 12287)

                level_3 <= 1'b1;

            else if (angle_out >= 12287 && angle_out <= 16384)
            ...
            // clear levels if no valid angle
...
always @(posedge clk) begin
        ...
        if (rst) begin
            ...
            if (angle_out <= NEGTHRESHOLD && angle_out >-16'd4095)

               level_neg1 <= 1'b1;

            else if (angle_out <= -16'd4095 && angle_out >-16'd8191)

               level_neg2<= 1'b1;

            else if (angle_out <= -16'd8191 && angle_out >-16'd12287)

               level_neg3 <= 1'b1;

            else if (angle_out <= -16'd12287 && angle_out >= -16'd16384)
...
// Count up to 13, negate clk_reg, count up to 25, negate clk_reg,

// reset counter, repeat
...
module spi_master(
    ...
    output signed [15:0] accel_x,  // Full 16-bit signed X data

    output signed [15:0] accel_y,  // Full 16-bit signed Y data

    output signed [15:0] accel_z,  // Full 16-bit signed Z data

    output data_valid              // Indicates when new data is ready
...
// Full 16-bit registers for X, Y, Z data

reg [15:0] X = 16'b0;               // X data MSB and LSB from sensor

reg [15:0] Y = 16'b0;               // Y data MSB and LSB from sensor

reg [15:0] Z = 16'b0;               // Z data MSB and LSB from sensor
 
// Latched output registers - full 16-bit signed

reg signed [15:0] X_latched = 16'b0;

reg signed [15:0] Y_latched = 16'b0;

reg signed [15:0] Z_latched = 16'b0;
...
// Data Buffer - Latch full 16-bit signed values

always @(negedge iclk)

    if(latch_data) begin   // latch 1&1/2 tick after entering state END_SPI

        X_latched <= X[15:0];  // Full 16-bit signed X

        Y_latched <= Y[15:0];  // Full 16-bit signed Y

        Z_latched <= Z[15:0];  // Full 16-bit signed Z

    end
 
// Output full 16-bit signed accelerometer data

assign accel_x = X_latched;

assign accel_y = Y_latched;

assign accel_z = Z_latched;
 
// Data valid signal

assign data_valid = latch_data;
